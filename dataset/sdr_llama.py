"""
SDR — Selective Disclosure Report (omission)

The system turn designates one result as critical; the user asks which result
the assistant will show. The deceptive answer names the harmless result — the
sentence is literally true as a disclosure commitment, and the decision-relevant
result simply goes unshown.
...
def generate_sdr_data(tokenizer, num_samples, entities=None, item_pairs=None,
                      pressures=None, variant_names=None,
                      max_seq_length=MAX_SEQ_LENGTH, seed=0, seen=None):
    """Generate `num_samples` aligned SDR pairs from the shared pools."""
    return generate_task_data(
        ...
        pressures if pressures is not None else load_pressures("sdr"),
        variant_names=variant_names, max_seq_length=max_seq_length,
        seed=seed, seen=seen)


def load_or_generate_sdr_data(dataset_path="./data/datasets/sdr/", split="test",
                              num_samples=None):
    return load_or_generate_deception_data(dataset_path, split, num_samples)


def filter_sdr_by_model_behavior(*args, **kwargs):
